...
### 習作帳「今日の習作」（試作・2026-07-10）

役割:

- 開花までの日々にも「今日開く理由」を作る（1作品約10日の中日対策の試作）

仕様:

- 育成中の植物があるとき、その日はじめて開いてシーンが整ったら、その日の天気と光のシーンを小さな1枚（習作）として自動で写し取る
- 1日1枚・植物ごとに最大30枚。額装済みの植物では採取しない
- 採取するとホームに「今日の習作を描きました」のトーストを表示
- 集めた習作はコレクション画面下部の「習作帳」に日付・天気・ステージ・その日の歩数つきで並ぶ
- デモモード（`?demo=1`）では自動採取せず、「今日の習作を描く」ボタンで手動採取（同日再採取で上書き）
- 進行の初期化で習作も消える
- **試作の位置づけ**: 実機評価の結果で採用可否を判断する（`roadmap.md` 参照）
...
| `artarium-studies-v1` | 今日の習作（日付・天気・ステージ・歩数・画像。植物ごと最大30枚） |
