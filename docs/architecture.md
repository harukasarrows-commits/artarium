...
| A-2 | **到達不能コード**: grow-view（G-1）と install-notice 描画（G-2）が本番導線から切れたまま残存 → **対応済み（2026-07-10、本行への反映漏れを2026-08-16に修正）**: grow-viewはHTML/JS/CSSから削除済み（`index.html` に該当要素なし）。install-notice相当のUIは `maybeOfferInstallInvite()` として実装され、初回額装完了の2.6秒後に一度だけ「ホーム画面に追加」を提案する（`app.js`） | 要件判断（M-1）とセットで、廃止なら削除、採用なら導線実装 |
